...
export default function Projects({ projects, sub_category_name }) {
  ...
  const formattedName = sub_category_name
    ? sub_category_name.charAt(0).toUpperCase() + sub_category_name.slice(1).replace('_', ' ')
    : '';
...
export async function getStaticPaths() {
  return {
    paths: Object.keys(subCategoryMap).map(name => ({
      params: { sub_category_name: name },
    })),
    fallback: 'blocking',
  };
}
...
export async function getStaticProps({ params }) {
  ...
  try {
    ...
    return {
      props: {
        projects: convertBigInt(projectsList),
        sub_category_name,
      },
      revalidate: 3600,
    };
